...
function tgSendWatchdog_(text) {
  const p = PropertiesService.getScriptProperties();
  const token = p.getProperty(WD_BOT_TOKEN);
  const chatId = p.getProperty(WD_CHAT_ID) || p.getProperty('CHAT_ID');
...
function watchdog_sendSessionPing_(sessionId, status) {
  const sp = PropertiesService.getScriptProperties();
  const token = sp.getProperty(WD_BOT_TOKEN);
  const chatId = sp.getProperty(WD_CHAT_ID) || sp.getProperty('CHAT_ID');
