...
const VERSION = "door-production-v1";
const SCOPE_PATH = (() => {
...
self.addEventListener("install", (event) => {
  event.waitUntil(caches.open(STATIC_CACHE).then((cache) => cache.addAll(STATIC)));
  self.skipWaiting();
});
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.pathname.startsWith("/api/door/production/")) {
  ...
  if (url.pathname.startsWith(SCOPE_PATH) || url.pathname.startsWith("/pwa/door/icon-")) {
