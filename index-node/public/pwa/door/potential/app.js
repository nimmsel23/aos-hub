...
// ===== GAS Helper: POST with text/plain to avoid CORS preflight =====
// GAS Web Apps don't handle OPTIONS requests. Using text/plain skips preflight.
// GAS still parses e.postData.contents as JSON regardless of Content-Type.
async function gasPost(payload, timeoutMs = 15000) {
  const controller = new AbortController();
  ...
  try {
    ...
    try { return JSON.parse(text); } catch { return { ok: false, error: "invalid_json" }; }
  } catch (err) {
    clearTimeout(tid);
    throw err.name === "AbortError" ? new Error("GAS timeout") : err;
  }
}

async function gasQueue(entry) {
  const result = await gasPost({
    title: entry.title,
    description: entry.description || "",
    source: entry.source || "pwa-gas-fallback",
    capturedAt: entry.created_at || new Date().toISOString(),
  });
  if (!result.ok) throw new Error(result.error || "GAS queue failed");
  return result;
}
...
async function captureIdeas() {
  ...
  try {
    ...
    // Bridge failed: try GAS fallback (one entry per idea for clean sheet rows)
    try {
      ...
      if (gasOk === 0) throw new Error("all GAS posts failed");
      refs.captureInput.value = "";
      setStatus(`${gasOk} idea${gasOk === 1 ? "" : "s"} queued to cloud fallback.`, "warning");
    } catch (gasErr) {
...
// Sync recovery: pull gas_synced items from GAS → Bridge, then mark synced
async function syncRecoveryFromGAS() {
  if (!state.online) return;
  try {
    const resp = await fetch(GAS_URL + '&action=get_gas_synced');
    ...
    for (const item of data.items) {
      try {
        ...
        // Mark as synced in GAS (GET — no CORS issue)
        await fetch(GAS_URL + '&action=mark_synced&rowId=' + encodeURIComponent(item.rowId));
        recovered++;
      } catch (err) {
    ...
    if (recovered > 0) {
      await loadRemoteItems();
      setStatus(`Recovered ${recovered} item${recovered === 1 ? "" : "s"} from cloud.`, "ok");
    }
  } catch (err) {
