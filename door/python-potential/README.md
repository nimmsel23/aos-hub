...
**Symlink to ~/bin:**
```bash
ln -s ~/aos-hub/door/python-potential/hot.py ~/bin/hot
chmod +x ~/aos-hub/door/python-potential/hot.py
```

**Or add alias to .bashrc / .zshrc:**
```bash
alias hot='python3 ~/aos-hub/door/python-potential/hot.py'
```
...
- ✅ Creates .md file in `~/vault/Door/1-Potential/`
- ✅ Adds to `hotlist_index.json` (Source of Truth)
- ✅ Creates Taskwarrior task (project:Potential)
