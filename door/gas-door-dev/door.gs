...
function doorSetProp(key, value) {
  var allowed = {};
  allowed[DOOR_CONFIG.FOLDER_PROP] = true;
  allowed[DOOR_CONFIG.SHEET_PROP] = true;
  allowed[DOOR_CONFIG.TICKTICK_TOKEN_PROP] = true;
  allowed[DOOR_CONFIG.TICKTICK_PROJECT_PROP] = true;
  if (!allowed[key]) {
    throw new Error('doorSetProp: blocked key');
  }
  PropertiesService.getScriptProperties().setProperty(key, value);
}

function doorGetBaseFolder_() {
...
function doorGetCentreFolder(userKey) {
  var key = '';
  try {
    key = (typeof door_normalizeUserKey_ === 'function')
      ? door_normalizeUserKey_(userKey || door_getActiveUserKey_())
      : String(userKey || '').trim();
  } catch (_) {
  ...
  if (!key) {
    throw new Error('Door Standalone: missing user key (?k=...)');
  }

  if (typeof door_requireRegisteredUserKey_ === 'function') {
    key = door_requireRegisteredUserKey_(key);
  }

  var base = doorGetBaseFolder_();
  var users = doorGetOrCreateSubfolder(base, 'Users');
  var root = doorGetOrCreateSubfolder(users, key);

  // Standard subfolders (keep stable across tools).
  doorGetOrCreateSubfolder(root, '0-Drafts');
  doorGetOrCreateSubfolder(root, '1-Potential');
  doorGetOrCreateSubfolder(root, '2-Plan');
  doorGetOrCreateSubfolder(root, '3-Production');
  doorGetOrCreateSubfolder(root, '4-Profit');
  return root;
}
...
function door_saveWarStackDraft_(payload, userKey) {
  var key = '';
  try {
    key = (payload && payload.userKey) ? String(payload.userKey || '').trim() : '';
  } catch (_) {
    key = '';
  }
  if (!key) key = String(userKey || '').trim();
  return (typeof door_withUserKey_ === 'function')
    ? door_withUserKey_(key, function() { return door_saveWarStackDraftImpl_(payload); })
    : door_saveWarStackDraftImpl_(payload);
}

function door_saveWarStackDraftImpl_(payload) {
...
function door_loadWarStackDraft_(sessionId, userKey) {
  var key = String(userKey || '').trim();
  return (typeof door_withUserKey_ === 'function')
    ? door_withUserKey_(key, function() { return door_loadWarStackDraftImpl_(sessionId); })
    : door_loadWarStackDraftImpl_(sessionId);
}

function door_loadWarStackDraftImpl_(sessionId) {
...
function door_clearWarStackDraft_(sessionId, userKey) {
  var key = String(userKey || '').trim();
  return (typeof door_withUserKey_ === 'function')
    ? door_withUserKey_(key, function() { return door_clearWarStackDraftImpl_(sessionId); })
    : door_clearWarStackDraftImpl_(sessionId);
}

function door_clearWarStackDraftImpl_(sessionId) {
  var id = String(sessionId || '').trim();
  if (!id) return { ok: false, error: 'sessionId missing' };
...
function door_saveProfitJson_(data, userKey) {
  var key = '';
  try {
    key = (data && data.userKey) ? String(data.userKey || '').trim() : '';
  } catch (_) {
    key = '';
  }
  if (!key) key = String(userKey || '').trim();
  return (typeof door_withUserKey_ === 'function')
    ? door_withUserKey_(key, function() { return door_saveProfitJsonImpl_(data); })
    : door_saveProfitJsonImpl_(data);
}

function door_saveProfitJsonImpl_(data) {
  ...
  var task = null;
  if (profitUuid && typeof bridge_taskModify_ === 'function' && typeof door_sideEffectsAllowed_ === 'function' && door_sideEffectsAllowed_()) {
...
// ------------------------------------------------------------
function saveDoorEntry(payload, userKey) {
  var key = '';
  try {
    key = (payload && payload.userKey) ? String(payload.userKey || '').trim() : '';
  } catch (_) {
  ...
  return (typeof door_withUserKey_ === 'function')
    ? door_withUserKey_(key, function() { return saveDoorEntry_(payload); })
    : saveDoorEntry_(payload);
}

function saveDoorEntry_(payload) {
  ...
  if (phase) {
    ...
    fullMd = door_logPhaseChange_(fullMd, phase, 'Saved via Door Standalone');
  }
