// ================================================================
// FRUITS MODULE (GAS)
// - Shared storage + logic for HQ WebApp and Telegram bot.
// - Primary storage is JSON in Drive (fruits_store.json).
// - Sheets are legacy and kept only for compatibility/debug.
// - Webapp + bot both read/write the same JSON store.
// ================================================================

// Central config and Script Properties keys.
const FRUITS_CONFIG = {
  ROOT_HTML: 'Fruits',
...
// Hardcoded defaults (Script Properties override when set).
// Fill these with your real values as needed.
const FRUITS_DEFAULTS = {
  CHAT_ID: '',
  WEBAPP_URL: '',
  BOT_TOKEN: ''
};

// Used for UI + bot messages.
const FRUITS_EMOJIS = ['🍎','🍌','🍇','🍉','🍓','🍒','🍍','🥝','🍊','🍏'];
...
function fruits_getPropOrDefault_(key, fallback) {
  const props = PropertiesService.getScriptProperties();
  const value = props.getProperty(key);
  return value ? value : fallback;
}

function fruits_getDefaultChatId_() {
  return fruits_getPropOrDefault_(FRUITS_CONFIG.PROP_DEFAULT_CHAT_ID, FRUITS_DEFAULTS.CHAT_ID);
}

function fruits_getBotToken_() {
  return fruits_getPropOrDefault_(FRUITS_CONFIG.PROP_BOT_TOKEN, FRUITS_DEFAULTS.BOT_TOKEN);
}
...
// -----------------------------------------------------------------------------
// Debug: print current Script Properties + Drive IDs
// -----------------------------------------------------------------------------
...
function fruits_getQuestionIndex_() {
  // Build a question -> section map for fast reply matching.
...
function fruits_findQuestionInText_(text) {
  // Used when users reply to a previous bot message (reply_to_message).
...
function fruits_initIfNeeded_() {
  // Ensure Drive folder + store file exist.
  const props = PropertiesService.getScriptProperties();
...
function fruits_init_() {
  // Create/locate folder and prime the JSON store.
...
function fruits_ensureCentreFolder_() {
  // Drive structure: Alpha_Game/Fruits (creates if missing).
...
function fruits_ensureSpreadsheet_(folder) {
  // Legacy sheet creator (no longer used by main flow).
...
function fruits_getDriveFolder_() {
  // Primary folder for JSON store + exports.
  const props = PropertiesService.getScriptProperties();
...
function fruits_getStoreFile_() {
  // Primary JSON store file; pick the "best" existing file if multiple exist.
...
function fruits_loadStore_() {
  // Serialized JSON store; lock protects concurrent writes from triggers/webapp.
...
function fruits_saveStore_(store) {
  // Persist JSON store and bump updated_at in one locked write.
  const lock = LockService.getScriptLock();
  lock.waitLock(20000);
  try {
    const file = fruits_getStoreFile_();
...
function fruits_getAnswersSheet_() {
  // Legacy sheet (not used by main flow).
...
function fruits_loadAnswers_() {
  // Flatten store.answers to a simple {question: answer} map for UI.
  const answers = {};
...
function fruits_getSkippedQuestion_() {
  // Skip is stored in Script Properties (single skip at a time).
  const props = PropertiesService.getScriptProperties();
...
function fruits_hasPendingSkippedAnswer_(answers, skipped) {
  // If any answer is '_geskippt_' there is a pending skip to resolve.
...
function fruits_registerUser_(chatId, username) {
  // Track per-user state so bot knows last_question for each chat.
  const store = fruits_loadStore_();
...
function fruits_getLastQuestion_(chatId) {
  // Read last asked question for a chat from JSON store.
  const store = fruits_loadStore_();
  const id = String(chatId || '');
...
function fruits_setLastQuestion_(chatId, section, question) {
  // Update last_question for a chat (used by bot reply handling).
  const store = fruits_loadStore_();
  const id = String(chatId || '');
  if (!id) return;
  const users = store.users || {};
...
function fruits_getAllData() {
  // Webapp payload: questions + answers + metadata.
...
function fruits_saveAnswerWithMeta(section, question, answer, source, chatId) {
  // Central save path for webapp + telegram; handles skip state and logs.
...
function fruits_exportCompleteMap() {
  // Export full map as Markdown to Drive (adds an entry to store.logs).
  fruits_initIfNeeded_();
...
function fruits_saveSessionToDrive_(mdContent, filename) {
  // Save export to Drive and log it in JSON store.
  const folder = fruits_getDriveFolder_();
...
function fruits_getWebUrl_() {
  // Script property overrides, then hardcoded default, then deployed URL.
  const configured = fruits_getPropOrDefault_(FRUITS_CONFIG.PROP_WEBAPP_URL, FRUITS_DEFAULTS.WEBAPP_URL);
...
function fruits_sendNextQuestionToUser_(chatId) {
  // Pick the next unanswered question (or pending skip) and update last_question.
...
function fruits_skipCurrentQuestion_(chatId) {
  // Only allow one skip at a time; skip is stored as '_geskippt_'.
...
function fruits_setupDailyTrigger(hourUTC) {
  // One daily trigger for fruits_dailyQuestion (UTC hour).
...
function fruits_setupWebhook(deployUrl) {
  // Helper wrapper for setting webhook.
  const token = fruits_getBotToken_();
...
function fruits_setupPollingTrigger(minutes) {
  // Poll Telegram every N minutes (used when webhook is disabled).
...
function fruits_disablePolling_() {
  // Remove polling trigger and reset last update id.
  const triggers = ScriptApp.getProjectTriggers();
  triggers.forEach((t) => {
    if (t.getHandlerFunction() === 'fruits_pollTelegram_') {
      ScriptApp.deleteTrigger(t);
...
function fruits_setupComplete(config) {
  // Convenience wrapper to setup storage + token + webhook + daily trigger.
  const cfg = config || {};
  const results = {};
...
function fruits_dailyQuestion() {
  // Daily dispatcher; guard prevents multiple sends per day.
  if (fruits_dailyGuard_()) {
    return;
  }
  fruits_initIfNeeded_();
  ...
  if (!sent) {
    const fallback = fruits_getDefaultChatId_() || CONFIG.CHAT_ID;
    if (fallback) fruits_sendNextQuestionToUser_(fallback);
  }
}

function fruits_dailyGuard_() {
  // One send per day guard to prevent spam if triggers duplicate.
  const propKey = 'FRUITS_LAST_SENT_DATE';
  const props = PropertiesService.getScriptProperties();
  const last = props.getProperty(propKey) || '';
  const today = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd');
  if (last === today) {
    Logger.log('fruits_dailyGuard_: already sent today');
    return true;
  }
...
function fruits_resetDailyGuard_() {
  PropertiesService.getScriptProperties().deleteProperty('FRUITS_LAST_SENT_DATE');
  return { ok: true };
}

function fruits_handleTelegramMessage_(message) {
  // Telegram commands + reply-aware answer capture.
