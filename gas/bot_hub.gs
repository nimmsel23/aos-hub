...
function weeklyWarStackReminder() {
  ...
  try {
    ...
    // Weekly reminder message sent to main chat.
    const message = `⚔️ *Weekly War Stack - ${week}*\n\n` +
      ...
      `Or open ${CONFIG.WARSTACK_BOT_USERNAME} and send /war\n\n` +
...
function weeklyFireMapAutomation() {
  ...
  try {
    ...
    // Trigger Bridge automation for weekly firemap.
...
function hub_sendMessage_(chatId, text, replyMarkup) {
  ...
  // Only set parse_mode when explicitly requested.
  if (replyMarkup && typeof replyMarkup === 'object') {
    if (replyMarkup.parse_mode) {
      payload.parse_mode = replyMarkup.parse_mode;
    }
  } else if (replyMarkup === 'Markdown' || replyMarkup === 'HTML') {
    payload.parse_mode = replyMarkup;
  }

  if (replyMarkup && typeof replyMarkup === 'object' && replyMarkup.reply_markup) {
