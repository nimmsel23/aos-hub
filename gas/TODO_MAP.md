...
## P0 / Fix
- [x] Define `bridge_syncPull` and its trigger/command; now implemented for `/pull` + `setupBridgePullTrigger`. `bridge.gs`
...
- [ ] Prune unused reflection/task-sync blocks if no longer used (Reflection system, Task Sync Mapping). `gas/reflection.gs`, `gas/bridge.gs`
- [ ] Verify if `webapp_logCore4ToSheets`, `bridge_handleTelegramMessage`, `promoteTooorWar` are used; rename/remove if unused. `gas/hq_webapp.gs`, `gas/bridge.gs`, `gas/reflection.gs`
