...
// Polling is used instead of webhooks to keep this bot isolated.
...
function fireBot_buildMessagesFromTaskExport_(days) {
  ...
  // Scope is based on task_export snapshot (no live API calls).
...
function fireBot_getToken_() {
  // Use Fire bot token if set, otherwise fall back to shared bot token.
