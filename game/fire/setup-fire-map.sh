...
Usage:
  game/fire/setup-fire-map.sh

Options:
  --systemd   write+enable user timers via game/fire/firectl (recommended; scripts/firectl works too)
...
if [[ "${1:-}" == "--systemd" ]]; then
  if [[ -x "$ROOT_DIR/game/fire/firectl" ]]; then
    exec "$ROOT_DIR/game/fire/firectl" setup-systemd
  fi
  if [[ -x "$ROOT_DIR/scripts/firectl" ]]; then
    exec "$ROOT_DIR/scripts/firectl" setup-systemd
  fi
  die "missing: $ROOT_DIR/game/fire/firectl (or compat wrapper: $ROOT_DIR/scripts/firectl)"
fi
...
msg "Next:"
msg "  task fire"
msg "  game/fire/firectl setup-systemd   # optional timers"
