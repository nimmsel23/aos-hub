...
def pending_core4_today() -> List[Dict]:
    return task_export(["project:core4", "+core4", "due:today", "status:pending"])


def completed_core4_today(subtask: str) -> List[Dict]:
    return task_export([f"+core4", f"+{subtask}", "status:completed", "end:today"])
...
def handle_push() -> int:
    ...
    for subtask in SUBTASKS:
        tasks = completed_core4_today(subtask)
        if not tasks:
            continue
        for tw_task in tasks:
            tw_uuid = tw_task.get("uuid")
            if not tw_uuid or tw_uuid not in mapping:
                continue
