# GAS HQ (alphaos_single_project.gs) Overview

## Entry Points
- `doGet()`: Renders `Index.html` (HQ UI).
- `doPost(e)`: Handles Telegram webhook + special payloads:
...
- `getSystemStatus_()`: Returns bridge/router/heartbeat/server info.
  - Bridge: `/health` on `AOS_BRIDGE_URL` (or `BRIDGE_URL`/`LAPTOP_WEBHOOK_URL`), with auth headers if set.
  - Router: `ROUTER_HEARTBEAT_TS` age.
  - Heartbeat: `WATCHDOG_LAST_BEAT_TS` age.
  - Bridge fallback: `BRIDGE_HEARTBEAT_TS` (hb Xm).
  - Sends a Telegram ping per load with session id + labels.
- `debugStatus()`: Logs current status (Bridge/Router/Heartbeat/Server) to the GAS console.
- `getDashboardStats_()`: Hotlist/WaR Stack counters.
...
- `renderInlineMapHtml_(key)`: pulls inline templates.
- `getCentreUrls_()`: provides external URLs for buttons/dots (if set).
- `getBridgeUrl_()`: central bridge URL resolution (AOS_BRIDGE_URL/BRIDGE_URL/LAPTOP_WEBHOOK_URL/LAPTOP_URL).
...
- Reminders/Timers:
  - `weeklyWarStackReminder()` → Telegram reminder (no markdown parse_mode).
  - `weeklyFireMapAutomation()` → calls `CONFIG.LAPTOP_URL + /trigger/weekly-firemap`.
  - `dailyReview()` → calls `CONFIG.LAPTOP_URL + /daily-review-data` (requires laptop endpoint; else sends “Data Unavailable”).
