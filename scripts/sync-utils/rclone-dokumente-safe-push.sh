#!/usr/bin/env bash
# Safe daily rclone push for ~/Dokumente (copy, no deletes; skip symlinks by default).
#
# Env:
#   AOS_DOKUMENTE_LOCAL_PATH=$HOME/Dokumente
#   AOS_DOKUMENTE_REMOTE=eldanioo:Dokumente
#   AOS_COPY_LINKS=0|1
#   AOS_DRY_RUN=0|1
#   AOS_RCLONE_CONFIG=$HOME/.config/rclone/rclone.conf
#   AOS_LOG_DIR=$HOME/.local/share/alphaos/logs
#   AOS_DOKUMENTE_LOCK_FILE=/tmp/rclone-dokumente-safe-push.lock
#
# Args:
#   --dry-run

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "${SCRIPT_DIR}/common.sh"

LOCAL_PATH="${AOS_DOKUMENTE_LOCAL_PATH:-$HOME/Dokumente}"
REMOTE="${AOS_DOKUMENTE_REMOTE:-eldanioo:Dokumente}"
RCLONE_CONFIG="${AOS_RCLONE_CONFIG:-$HOME/.config/rclone/rclone.conf}"
LOG_DIR="${AOS_LOG_DIR:-$HOME/.local/share/alphaos/logs}"
LOG_FILE="${AOS_DOKUMENTE_LOG_FILE:-$LOG_DIR/alphaos-dokumente-safe-push.log}"
LOCK_FILE="${AOS_DOKUMENTE_LOCK_FILE:-/tmp/rclone-dokumente-safe-push.lock}"

COPY_LINKS_MODE="${AOS_DOKUMENTE_COPY_LINKS:-${AOS_COPY_LINKS:-0}}"
DRY_RUN="${AOS_DRY_RUN:-0}"

parse_args() {
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --dry-run|--dry) DRY_RUN=1; shift ;;
      *) shift ;;
    esac
  done
}

is_remote_root() {
  case "$REMOTE" in
    *:) return 0 ;;
    *:/) return 0 ;;
  esac
  return 1
}

acquire_lock() {
  mkdir -p "$(dirname "$LOG_FILE")"
  if command -v flock >/dev/null 2>&1; then
    exec 9>"$LOCK_FILE"
    if ! flock -n 9; then
      warning "Another dokumente push is already running (lock: $LOCK_FILE)"
      exit 0
    fi
    return 0
  fi

  if [ -f "$LOCK_FILE" ]; then
    warning "Another dokumente push may be running (lock file exists: $LOCK_FILE)"
    exit 0
  fi
  touch "$LOCK_FILE"
  trap 'rm -f "$LOCK_FILE"' EXIT
}

main() {
  parse_args "$@"
  acquire_lock

  if [ ! -f "$RCLONE_CONFIG" ]; then
    warning "rclone config missing; nothing to do ($RCLONE_CONFIG)"
    exit 0
  fi

  if [ ! -d "$LOCAL_PATH" ]; then
    error "Local path not found: $LOCAL_PATH"
    exit 1
  fi

  if is_remote_root; then
    error "Refusing to push to remote root: '$REMOTE' (set AOS_DOKUMENTE_REMOTE to a folder, e.g. eldanioo:Dokumente)"
    exit 1
  fi

  local -a rclone_opts=(
    --config "$RCLONE_CONFIG"
    --create-empty-src-dirs
    --fast-list
    --log-file "$LOG_FILE"
    --log-level INFO
    --exclude "**/.git/**"
    --exclude "**/node_modules/**"
    --exclude "**/__pycache__/**"
    --exclude "**/.venv/**"
    --exclude "**/.pytest_cache/**"
    --exclude "**/.mypy_cache/**"
    --exclude "**/.ruff_cache/**"
    --exclude "**/.cache/**"
    --exclude "**/.Trash-*/**"
    --exclude "**/.DS_Store"
    --exclude "**/Thumbs.db"
    --exclude "**/*.tmp"
  )

  if [ "$COPY_LINKS_MODE" = "1" ]; then
    rclone_opts+=(--copy-links)
  else
    rclone_opts+=(--skip-links)
  fi

  if [ "$DRY_RUN" = "1" ]; then
    log "Dry-run: pushing Dokumente to $REMOTE (copy)"
    rclone copy "$LOCAL_PATH" "$REMOTE" "${rclone_opts[@]}" --dry-run
  else
    log "Pushing Dokumente to $REMOTE (copy)"
    rclone copy "$LOCAL_PATH" "$REMOTE" "${rclone_opts[@]}"
  fi

  success "Dokumente push completed"
}

main "$@"

